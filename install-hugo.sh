...
curl -Ls https://github.com/${HUGO_REPO}/releases/download/${HUGO_VERSION}/hugo_extended_${HUGO_VERSION:1}_Linux-64bit.tar.gz \
     | tar -xz -C $SANDBOX_CONF_HOME/bin hugo
mv $SANDBOX_CONF_HOME/bin/hugo $SANDBOX_CONF_HOME/bin/hugox
curl -Ls https://github.com/${HUGO_REPO}/releases/download/${HUGO_VERSION}/hugo_${HUGO_VERSION:1}_Linux-64bit.tar.gz \
     | tar -xz -C $SANDBOX_CONF_HOME/bin hugo
